...
public class TNGCustomAPInew implements YIFCustomApi {
	...
	public Document createOrderPick(YFSEnvironment env, Document inDoc) throws Exception {
        this.debug("TNGCustomAPI: service input : " + YFCDocument.getDocumentFor(inDoc));
        String delMethod = XPathAPI.eval(inDoc.getDocumentElement(),"/Order/OrderLines/OrderLine/@DeliveryMethod").toString();
        Document outDoc;
        if (delMethod.equals("PICK"))
        {
         outDoc = invokeAPI(env, "global/template/api/createorder.xml", "createOrder", inDoc);
        }
        else
        {
         outDoc = invokeAPI(env, null, "createOrder", inDoc);
        }
        return outDoc;
        }
	...
	private void debug(String aMsg) {
		if (TNGCustomAPInew.mIsDebugEnabled) {
			TNGCustomAPInew.mLogger.debug(aMsg);
		}
	}
